...
func TestCrossModelQuery(t *testing.T) {
	...
	mockClient.EXPECT().CrossModelQuery(gomock.Any()).Return(&params.CrossModelQueryResponse{
		Results: map[string][]params.ModelQueryResult{
			"model-1": {},
		},
...
func TestCrossModelQueryAPIError(t *testing.T) {
	c := qt.New(t)
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
...
func TestCrossModelQueryError(t *testing.T) {
	c := qt.New(t)
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	// Setup expectations
	mockStore.EXPECT().CurrentController().Return("test-controller", nil)
	mockClient.EXPECT().CrossModelQuery(gomock.Any()).Return(nil, expectedErr)
	mockClient.EXPECT().Close().Return(nil)
