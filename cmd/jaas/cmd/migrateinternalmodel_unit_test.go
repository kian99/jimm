...
func TestMigrateInternalModel(t *testing.T) {
	c := qt.New(t)
	...
	mockClient.EXPECT().MigrateModel(gomock.Any()).Return(&jujuparams.InitiateMigrationResults{}, nil)
	...
	command := &migrateInternalModelCommand{
		...
		modelTargets:     []string{"ac30d6ae-0bed-4398-bba7-75d49e39f189"},
...
func TestMigrateInternalModelAPIError(t *testing.T) {
	c := qt.New(t)
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
