...
func controllerConnectionFunc(s apiModelProxier, jwtGenerator *jujuauth.LoginTokenGenerator) func(context.Context) (rpcproxy.WebsocketConnectionWithMetadata, error) {
	return func(ctx context.Context) (rpcproxy.WebsocketConnectionWithMetadata, error) {
		...
		if err != nil {
			return rpcproxy.WebsocketConnectionWithMetadata{}, errors.Newf("error parsing path: %w", err)
		}
		m := dbmodel.Model{
			UUID: sql.NullString{
				String: uuid,
		...
		if err := s.jimm.Database.GetModel(context.Background(), &m); err != nil {
			return rpcproxy.WebsocketConnectionWithMetadata{}, errors.Newf("failed to find model: %w", err).WithCode(errors.CodeNotFound)
		}
