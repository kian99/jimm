...
func (as *AuthenticationService) AuthCodeURL() (string, string, error) {
	...
	_, err := rand.Read(b)
	if err != nil {
		return "", "", errors.Newf("failed to generate state secret: %s", err.Error())
	}
...
func (as *AuthenticationService) Logout(ctx context.Context, w http.ResponseWriter, req *http.Request) error {
	...
	if !ok {
		return errors.Newf("session identity key could not be parsed: expected %T, got %T", identityIdStr, identityId)
	}
...
func (as *AuthenticationService) validateAndUpdateAccessToken(ctx context.Context, email any) error {
	...
	if !ok {
		return errors.Newf("failed to cast email: got %T, expected %T", email, emailStr)
	}
