...
func (d *Database) GetSecret(ctx context.Context, secret *dbmodel.Secret) (err error) {
	...
	if secret.Tag == "" || secret.Type == "" {
		return errors.New("missing secret tag and type").WithCode(errors.CodeBadRequest)
	}
	...
	if err := db.First(&secret).Error; err != nil {
		...
		if errors.ErrorCode(err) == errors.CodeNotFound {
			return errors.Wrap(err).WithMessage("secret not found")
		}
		return dbError(err)
	}
	return nil
...
func (d *Database) Get(ctx context.Context, tag names.CloudCredentialTag) (_ map[string]string, err error) {
	...
	if err != nil {
		...
		return nil, errors.Newf("failed to get secret data: %w", err)
	}
	var attr map[string]string
	err = json.Unmarshal(secret.Data, &attr)
	if err != nil {
		return nil, errors.Newf("failed to unmarshal secret data: %w", err)
	}
	return attr, nil
}
...
func (d *Database) Put(ctx context.Context, tag names.CloudCredentialTag, attr map[string]string) (err error) {
	...
	if err != nil {
		return errors.Newf("failed to marshal secret data: %w", err)
	}
...
func (d *Database) GetControllerCredentials(ctx context.Context, controllerName string) (_ string, _ string, err error) {
	...
	if err != nil {
		return "", "", errors.Newf("failed to get secret data: %w", err)
	}
	var secretData map[string]string
	err = json.Unmarshal(secret.Data, &secretData)
	if err != nil {
		return "", "", errors.Newf("failed to unmarshal secret data: %w", err)
	}
...
func (d *Database) CleanupJWKS(ctx context.Context) (err error) {
	...
	if err != nil {
		return errors.Newf("failed to cleanup jwks public key: %w", err)
	}
	secret = dbmodel.NewSecret(jwksKind, jwksPrivateKeyTag, nil)
	err = d.DeleteSecret(ctx, &secret)
	if err != nil {
		return errors.Newf("failed to cleanup jwks private key: %w", err)
	}
	secret = dbmodel.NewSecret(jwksKind, jwksExpiryTag, nil)
	err = d.DeleteSecret(ctx, &secret)
	if err != nil {
		return errors.Newf("failed to cleanup jwks expiry info: %w", err)
	}
	return nil
}
...
func (d *Database) GetJWKS(ctx context.Context) (_ jwk.Set, err error) {
	...
	err = d.GetSecret(ctx, &secret)
	if err != nil {
		return nil, errors.Newf("failed to get jwks data: %w", err)
	}
	ks, err := jwk.ParseString(string(secret.Data))
	if err != nil {
		return nil, errors.Newf("failed to parse jwk data: %w", err)
	}
	return ks, nil
}
...
func (d *Database) GetJWKSPrivateKey(ctx context.Context) (_ []byte, err error) {
	...
	err = d.GetSecret(ctx, &secret)
	if err != nil {
		return nil, errors.Newf("failed to get jwks private key: %w", err)
	}
	var pem []byte
	err = json.Unmarshal(secret.Data, &pem)
	if err != nil {
		return nil, errors.Newf("failed to unmarshal pem data: %w", err)
	}
	return pem, nil
}
...
func (d *Database) GetJWKSExpiry(ctx context.Context) (_ time.Time, err error) {
	...
	err = d.GetSecret(ctx, &secret)
	if err != nil {
		return time.Time{}, errors.Newf("failed to get jwks expiry: %w", err)
	}
	var expiryTime time.Time
	err = json.Unmarshal(secret.Data, &expiryTime)
	if err != nil {
		return time.Time{}, errors.Newf("failed to unmarshal jwks expiry data: %w", err)
	}
...
func (d *Database) PutJWKS(ctx context.Context, jwks jwk.Set) (err error) {
	...
	if err != nil {
		return errors.Newf("failed to marshal jwks data: %w", err)
	}
...
func (d *Database) PutJWKSPrivateKey(ctx context.Context, pem []byte) (err error) {
	...
	if err != nil {
		return errors.Newf("failed to marshal jwks private key: %w", err)
	}
...
func (d *Database) PutJWKSExpiry(ctx context.Context, expiry time.Time) (err error) {
	...
	if err != nil {
		return errors.Newf("failed to marshal jwks expiry data: %w", err)
	}
...
func (d *Database) CleanupOAuthSecrets(ctx context.Context) (err error) {
	...
	if err := d.DeleteSecret(ctx, &secret); err != nil {
		return errors.Newf("failed to cleanup OAuth key: %w", err)
	}
	secret = dbmodel.NewSecret(oauthKind, oauthSessionStoreSecretTag, nil)
	if err := d.DeleteSecret(ctx, &secret); err != nil {
		return errors.Newf("failed to cleanup OAuth session store secret: %w", err)
	}
	return nil
}
