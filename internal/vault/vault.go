...
func (s *VaultStore) GetJWKS(ctx context.Context) (_ jwk.Set, err error) {
	...
	if secret == nil || secret.Data == nil {
		...
		return nil, errors.Wrap(msg).WithCode(errors.CodeNotFound)
	}
...
func (s *VaultStore) GetJWKSExpiry(ctx context.Context) (_ time.Time, err error) {
	...
	if secret == nil || secret.Data == nil {
		...
		zapctx.Debug(ctx, msg)
		return now, errors.Wrap(msg).WithCode(errors.CodeNotFound)
	}
...
func (s *VaultStore) client(ctx context.Context) (*api.Client, error) {
	...
	if err != nil {
		return nil, errors.Newf("unable to initialize approle auth method: %w", err)
	}

	authInfo, err := s.Client.Auth().Login(ctx, appRoleAuth)
	if err != nil {
		return nil, errors.Newf("unable to login to approle auth method: %w", err)
	}
	if authInfo == nil {
