...
func (s *identityManagerSuite) TestListIdentities(c *qt.C) {
	...
	users, err := s.manager.ListIdentities(ctx, u, pag, "")
	c.Assert(err, qt.IsNil)
	...
	for _, t := range testCases {
		c.Run(t.desc, func(c *qt.C) {
			...
			identities, err := s.manager.ListIdentities(ctx, u, pag, t.match)
			c.Assert(err, qt.IsNil)
...
func (s *identityManagerSuite) TestCountIdentities(c *qt.C) {
	...
	count, err := s.manager.CountIdentities(ctx, u)
	c.Assert(err, qt.IsNil)
	c.Assert(count, qt.Equals, 4)
}

func (s *identityManagerSuite) TestIdentityManager(t *testing.T) {
